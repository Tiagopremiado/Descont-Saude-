...
const IdCardView: React.FC<IdCardViewProps> = ({ name = '', role = 'TITULAR', cardNumber = '', holderName }) => {
    ...
    // Lógica para tamanho de fonte dinâmico baseada no tamanho do nome
    const getNameFontSize = (isZoomed: boolean, textLength: number) => {
        if (isZoomed) {
            if (textLength > 35) return 'text-lg leading-tight'; // Muito longo
            if (textLength > 25) return 'text-xl leading-tight'; // Longo
            return 'text-3xl leading-none'; // Normal
        } else {
            // Modo cartão pequeno (lista)
            if (textLength > 35) return 'text-[9px] leading-tight';
            if (textLength > 25) return 'text-[10px] leading-tight';
    ...
    const renderCardContent = (isZoomed: boolean) => {
        const nameFontSizeClass = getNameFontSize(isZoomed, name.length);
        ...
        return (
            ...
            >
                ...
                <div className={`absolute ${isZoomed ? 'bottom-8 left-10 right-10' : 'bottom-5 left-6 right-6'} z-10 transition-all`}>
                    <div className="flex flex-col">
                        {/* Role Label - Borda removida, Sombreado adicionado */}
                        <div className={`flex items-center gap-2 ${isZoomed ? 'mb-2' : 'mb-1'}`}>
                            <span 
                        ...
                        {/* Name - FONTE DINÂMICA E QUEBRA DE LINHA PERMITIDA */}
                        <h2 
                            className={`${nameFontSizeClass} font-medium text-white uppercase tracking-wide drop-shadow-md pb-1 transition-all`} 
                            style={{ 
                                fontFamily: '"Courier New", Courier, monospace', 
                                letterSpacing: '0.02em', 
                                textShadow: '1px 1px 2px rgba(0,0,0,0.6)',
                                wordBreak: 'break-word', // Permite quebra se necessário
                                display: '-webkit-box',
